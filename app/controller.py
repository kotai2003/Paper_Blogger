"""Controller - GUIとPipelineを接続するMVCコントローラー

GUI (Tkinter) と paper_blog_pipeline を接続する MVC コントローラーモジュール。
パイプラインの非同期実行、ファイル操作、設定管理を担当する。
"""
...
class PipelineController:
    """パイプライン実行を管理するMVCコントローラー

    GUI と paper_blog_pipeline を接続し、パイプラインの非同期実行を管理する。
    パイプラインはデーモンスレッドで実行され、GUI更新は ``root.after()`` 経由で
    メインスレッドに委譲される。
    ...
    root : tk.Tk
        Tkinter ルートウィンドウ
    main_window : MainWindow or None
        メインウィンドウへの参照（set_main_window で設定）
    """

    def __init__(self, root: tk.Tk):
        self.root = root
        self.main_window = None  # MainWindowから設定される
    ...
    def run_pipeline(self):
        ...
        左パネルから設定とPDFパスを取得し、デーモンスレッドで
        7ステップのパイプラインを実行する。実行中は進捗バーと
        ログコンソールがリアルタイムで更新される。

        Raises
        ------
        messagebox
            PDFが選択されていない場合、またはパイプラインが既に実行中の場合に
            エラーダイアログを表示する。
        """
    ...
    def _run_pipeline_worker(self, pdf_path: str, settings: dict):
        ...
        settings : dict
            GUI左パネルから取得した設定辞書。
            キー: model, vlm_model, base_url, max_figures, skip_figures, output_dir
        """
        try:
            # パイプラインモジュールのインポート
            ...
            # Step 1: PDF解析
            self._update_progress(1, 7, "Step 1/7: PDF Parsing...")
            self._log("Step 1/7: PDF parsing...", "step")
            ...
            # Step 2: 図抽出
            self._update_progress(2, 7, "Step 2/7: Figure Extraction...")
            self._log("Step 2/7: Extracting figures...", "step")
            ...
            if not skip_figures and key_figures:
                self._update_progress(3, 7, "Step 3/7: Figure Analysis (VLM)...")
                self._log(f"Step 3/7: Analyzing figures (VLM: {vlm_model})...", "step")
                ...
                self._update_progress(3, 7, "Step 3/7: Skipped")
                self._log("Step 3/7: Figure analysis skipped", "step")
            ...
            # Step 4: 論文理解
            self._update_progress(4, 7, "Step 4/7: Paper Analysis (LLM)...")
            self._log("Step 4/7: Analyzing paper (Ochiai method)...", "step")
            ...
            # Step 5: Insight生成
            self._update_progress(5, 7, "Step 5/7: Insight Generation...")
            self._log("Step 5/7: Generating insights...", "step")
            ...
            # Step 6: 落合式スライド
            self._update_progress(6, 7, "Step 6/7: Ochiai Summary...")
            self._log("Step 6/7: Generating Ochiai summary...", "step")
            ...
            # Step 7: ブログ記事生成
            self._update_progress(7, 7, "Step 7/7: Blog Article...")
            self._log("Step 7/7: Generating blog article...", "step")
    ...
    def _on_complete(self, cancelled: bool = False, error: bool = False):
        def _finish():
            ...
            if cancelled:
                ...
                self.main_window.toolbar.set_progress(0, 7, "Cancelled")
            elif error:
                self.main_window.status_bar.set_status("Error")
                self.main_window.toolbar.set_progress(0, 7, "Error")
            else:
                self.main_window.status_bar.set_status("Complete")
                self.main_window.toolbar.set_progress(7, 7, "Complete")
