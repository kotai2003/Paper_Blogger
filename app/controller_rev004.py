"""Controller Rev004 - 中国語フィルターを廃止した7ステップパイプラインコントローラー

Rev002 (controller_rev002.py) との差分:
  - Step 8 (中国語フィルター) を廃止 → 7ステップ構成に戻す
  - LLM/VLM モデルは GUI のプルダウン (ollama list 由来) から取得した値を使用
"""
...
class PipelineController:
    """パイプライン実行を管理するMVCコントローラー（Rev004: 7ステップ・中国語フィルター無し）
    ...
    TOTAL_STEPS = 7  # Rev004: 8→7 (中国語フィルター削除)
    ...
    def _run_pipeline_worker(self, pdf_path: str, settings: dict):
        """ワーカースレッドでパイプラインを実行（Rev004: 7ステップ・中国語フィルター無し）"""
        N = self.TOTAL_STEPS  # 7
        ...
        try:
            ...
            # Rev004: from llm.chinese_filter import filter_chinese を削除
            ...
            # Step 1: PDF解析
            self._update_progress(1, N, "Step 1/7: PDF Parsing...")
            ...
            # Step 2: 図抽出
            self._update_progress(2, N, "Step 2/7: Figure Extraction...")
            ...
            if not skip_figures and key_figures:
                self._update_progress(3, N, "Step 3/7: Figure Analysis (VLM)...")
                ...
                self._update_progress(3, N, "Step 3/7: Skipped")
            ...
            # Step 4: 論文理解
            self._update_progress(4, N, "Step 4/7: Paper Analysis (LLM)...")
            ...
            # Step 5: Insight生成
            self._update_progress(5, N, "Step 5/7: Insight Generation...")
            ...
            # Step 6: 落合式スライド
            self._update_progress(6, N, "Step 6/7: Ochiai Summary...")
            ...
            # Step 7: ブログ記事生成
            self._update_progress(7, N, "Step 7/7: Blog Article...")
    ...
    def open_settings(self):
        # Rev004: モデル情報は config.yaml ではなく ollama list から取得するため、
        # モデル以外のキー (base_url, max_figures, skip_figures) のみ load_config で反映される
        config_path = self._pipeline_dir / "config.yaml"
        if config_path.exists():
            with open(config_path, "r", encoding="utf-8") as f:
                config = yaml.safe_load(f) or {}
            # Rev004: YAML 側のモデル指定は無視する
            config.pop("model", None)
            config.pop("vlm_model", None)
            self.main_window.left_panel.load_config(config)
            self._log(f"Loaded config (model fields ignored): {config_path}", "info")
        else:
